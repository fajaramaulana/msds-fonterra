...
@section('content')
    ...
    <form enctype="multipart/form-data" id="form-listjasa">
        ...
        <div class="form-group">
            ...
            <select class="form-control " name="id_jasa" id="">
                ...
                @foreach ($jasas as $jasa)
                    <option value="{{ $jasa->id }}" {{ $jasa->id == $porto->id ? 'selected' : '' }}>
                        {{ $jasa->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            <label>Nama</label>
            <input type="text" class="form-control" value="{{ $porto->name }}" name="name" id="name">
        </div>
        @if ($porto->image != null)
            <div class="form-group">
                <label>Preview</label>
                <div class="preview-img">
                    <img src="{{ env('PATH_IMAGE') . $porto->image }}" style="width: 20%; height: auto;"
                        alt="{{ $porto->name }}">
                    <button class="btn btn-danger btn-sm"
                        onclick="event.preventDefault(); removeImage({{ $porto->id }})">X</button>
                </div>
            </div>
        @else
        ...
        <div class="form-group">
            <select class="form-control " name="status" id="">
                ...
                <option value="1" {{ $porto->status == 1 ? 'selected' : '' }}>Active</option>
                <option value="0" {{ $porto->status == 0 ? 'selected' : '' }}>Non Active</option>
            </select>
        </div>


        <div class="form-group">
            <button class="btn btn-primary btn-block">Update Portofolio</button>
        </div>
    </form>
@endsection
    ...
    <script type="text/javascript">
        removeImage = (id) => {
            ...
            swal({
                ...
                if (result == true) {
                    $.ajax({
                        url: '{{ route('portofoliomanagement.removeImage') }}',
                        type: 'POST',
                        data: formDataRemove,
                        processData: false,
                        contentType: false,
        ...
        $('#form-listjasa').submit(function(event) {
            ...
            $.ajax({
                url: '{{ route('portofoliomanagement.update', $porto->id) }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(result) {
                    ...
                    if (result.success == 1) {
                        swal({
                            ...
                            window.location.href = "{{ url('/portofoliomanagement') }}";
                        })
